...
class GridEncoder(nn.Module):
    ...
    def forward(self, inputs, bound=1, max_level=None, out_embeddings=None):
        ...
        # quant
        if not self.quant and self.quant_embeddings is None:
            embeddings = self.embeddings
        else:
            embeddings = self.quant_embeddings
            
        # print(embeddings[:4, :], self.embeddings[:4, :])
        if out_embeddings is None:
            outputs = grid_encode(inputs, embeddings, self.offsets, self.per_level_scale,
