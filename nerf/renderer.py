...
from spikingjelly.clock_driven.neuron import LIFNode, IFNode, ParametricLIFNode
import raymarching
...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, bg_color=None, perturb=False, cam_near_far=None, shading='full', **kwargs):
        ...
        if self.training:
            # rays: [N, 2], the sample point number of each ray
            ...
            while step < self.opt.max_steps:
                ...
                dirs = dirs / torch.norm(dirs, dim=-1, keepdim=True)
                with torch.amp.autocast('cuda', enabled=self.opt.fp16):
                    # print("Ray matching output: ", xyzs.shape, dirs.shape)
                    # print(f'Input: {xyzs.shape}, Dir: {dirs.shape}, Ts: {ts.shape}, Rays: {rays.shape}')
                    outputs = self(xyzs, dirs, shading=shading)
                    sigmas = outputs['sigma']
                    rgbs = outputs['color']
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            if torch.is_tensor(fx):
                                cx_div_fx = (cx[head:tail] / fx[head:tail]).to(cam_xyzs.device)
                                cy_div_fy = (cy[head:tail] / fy[head:tail]).to(cam_xyzs.device)
                            else:
                                cx_div_fx = cx / fx
                                cy_div_fy = cy / fy
                            
                            min_near = torch.tensor(self.opt.min_near).to(cam_xyzs.device) if cam_near_far is None else cam_near_far[head:tail, 0].unsqueeze(1)
                            min_near = min_near.to(cam_xyzs.device)
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        # print('updating extra states...')
        if not self.cuda_ray:
            return 
        
        ### update density grid
        # print('Updating density grid... ')
        with torch.no_grad():
            ...
            if self.iter_density < 16:
                # print(f'[density grid] full update at iter {self.iter_density}')
                X = torch.arange(self.grid_size, dtype=torch.int32, device=self.aabb_train.device).split(S)
                Y = torch.arange(self.grid_size, dtype=torch.int32, device=self.aabb_train.device).split(S)
                Z = torch.arange(self.grid_size, dtype=torch.int32, device=self.aabb_train.device).split(S)
                    ...
                    for ys in Y:
                        for zs in Z:
                            ...
                            for cas in range(self.cascade):
                                ...
                                NeRFRenderer.reset_net(self.sigma_net)
                                # assign 
        ...
        # np.save('valid_mask.npy', valid_mask.cpu().numpy())
        ...
        # convert to bitfield
        # print(f'[density grid] mean density = {self.mean_density}, thresh {self.density_thresh}')
        density_thresh = min(self.mean_density, self.density_thresh)
        density_thresh = min(density_thresh, self.density_grid.max().item() * 0.95)
    ...
    def reset_net(net):
        for item in net:
            if isinstance(item, LIFNode) or isinstance(item, IFNode) or isinstance(item, ParametricLIFNode):
                item.reset()
            return net
