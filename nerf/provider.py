...
class NeRFDataset:
    def __init__(self, opt, device, type='train', n_test=10):
        ...
        # nerf_synthetic dataset 
            ...
            print(os.path.join(self.root_path, 'transforms_train.json'))
    ...
    def collate(self, index):
        ...
        if self.training:
            ...
            if self.opt.random_image_batch:
                num_rays = max(4096, num_rays)
                index = torch.randint(0, len(self.poses), size=(num_rays,), device=self.device)
                # print('dataloader: ', num_rays, index.shape)

        else:
            num_rays = -1
        # print(self.poses.device, index)
        self.poses = self.poses.to(self.device)
        poses = self.poses[index]
        # poses = poses.to(self.device) # [N, 4, 4]
