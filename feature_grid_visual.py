...
if __name__ == '__main__':
    ...
    original_embeddings = torch.load('/home/linrx/Code/PATA_Quan/workspace/synthetic_nerf/ann/lego/checkpoints/ngp_ep0600.pth')['model']['encoder.embeddings']
    original_ema = torch.load('/home/linrx/Code/PATA_Quan/workspace/synthetic_nerf/ann/lego/checkpoints/ngp_ep0600.pth')['ema']['shadow_params']
    offsets = torch.load('/home/linrx/Code/PATA_Quan/workspace/synthetic_nerf/ann/lego/checkpoints/ngp_ep0600.pth')['model']['encoder.offsets']
    test_loader = NeRFDataset(opt, device=device, type='test').dataloader()
    # for i in range(8,16):
    #     with torch.no_grad():
    #         trainer.model.encoder.embeddings.copy_(original_embeddings)
    #         trainer.model.encoder.embeddings[offsets[i]:offsets[i+1], :].zero_()
    #         trainer.ema.shadow_params = original_ema
    #         trainer.ema.shadow_params[0][offsets[i]:offsets[i+1], :].zero_()
    ...
    #     trainer.test(test_loader, write_video=True, save_path=f'/home/linrx/Code/PATA_Quan/workspace/synthetic_nerf/ann/lego/ablation/{i}_level', offsets = offsets[i]) # test and save video
    ...
    with torch.no_grad():
        ...
        trainer.ema.shadow_params = original_ema
        trainer.ema.shadow_params[0][offsets[i]:offsets[i+1], :].zero_()
        print(f'[INFO] Ablation {i}-level: range [{offsets[i]}, {offsets[i+1]}]')
    trainer.workspace = f'/home/linrx/Code/PATA_Quan/workspace/synthetic_nerf/ann/lego/ablation/{i}_level'
    trainer.evaluate(valid_loader)
    
    if test_loader.has_gt:
        trainer.evaluate(test_loader) # blender has gt, so evaluate it.
    trainer.test(test_loader, write_video=True, save_path=f'/home/linrx/Code/PATA_Quan/workspace/synthetic_nerf/ann/lego/ablation/{i}_level/result', offsets = offsets[i]) # test and save video